...
const Header = ({ 
  ...
  return (
    <header className="bg-gray-900 border-b border-gray-700/30 px-3 sm:px-4 py-2 sm:py-3">
      <div className="flex items-center justify-between max-w-4xl mx-auto">
        {/* Left side with hamburger menu and title */}
        <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          <button
            onClick={onToggleSidebar}
            className="p-2 hover:bg-gray-800 rounded-lg transition-all duration-300 flex-shrink-0 touch-manipulation"
            aria-label="Toggle sidebar"
          >
            <HiBars3 className="w-5 h-5 sm:w-6 sm:h-6 text-white" />
          </button>
          
          <h1 className="text-white text-base sm:text-lg font-medium truncate">
            New chat
          </h1>
        </div>
        ...
        <div className="flex items-center gap-1 sm:gap-2 relative flex-shrink-0">
          ...
          <div className="relative hidden xs:block">
            <button
              ...
              className={`p-2 hover:bg-gray-800 rounded-lg transition-all duration-300 relative group touch-manipulation ${
                showContextualPrompts ? 'bg-gray-800 text-blue-400' : 'text-gray-300'
              }`}
              title="View Context & Prompts"
            >
              <HiBookmark className="w-4 h-4 sm:w-5 sm:h-5 group-hover:text-white transition-colors duration-200" />
              {messages.length > 1 && (
            ...
            {showContextualPrompts && (
              <div className="absolute right-0 top-full mt-2 w-72 sm:w-80 bg-gray-800 rounded-xl border border-gray-700/50 shadow-2xl z-50 max-h-80 sm:max-h-96 overflow-y-auto animate-scale-in">
                <div className="p-3 sm:p-4 border-b border-gray-700/50">
                  <div className="flex items-center gap-2 mb-2">
                    <HiChatBubbleLeftRight className="w-4 h-4 sm:w-5 sm:h-5 text-blue-400" />
                    <h3 className="text-base sm:text-lg font-bold text-white">Conversation Context</h3>
                  </div>
          ...
          {error && (
            <button
              onClick={onRetry}
              className="p-2 hover:bg-gray-800 rounded-lg transition-all duration-300 text-red-400 hover:text-red-300 touch-manipulation"
          ...
          <button
            onClick={onNewChat}
            className="p-2 hover:bg-gray-800 rounded-lg transition-all duration-300 text-gray-300 hover:text-white touch-manipulation"
          ...
          <button
            ...
            className="hidden sm:block p-2 hover:bg-gray-800 rounded-lg transition-all duration-300 text-gray-300 hover:text-white touch-manipulation"
